...
public class TESTSHOP_COMMENTDao {
	...
	public static int update(TESTSHOP_COMMENT comment) {
		...
		String sql = "update comment set USER_ID=?, GOODS_ID=?, COMMENT_PARENT=?, COMMENT_CONTENT=?, COMMENT_TIME=? where COMMENT_ID = ?";
	...
	public static ArrayList<TESTSHOP_COMMENT> selectAll(int cpage, int count, String keywords) {
		...
		try {
			if (keywords != null && !keywords.isEmpty()) {
				String sql = "select * from comment where USER_ID=? order by comment_ID asc limit ?, ?";
				ps = conn.prepareStatement(sql);
				ps.setString(1, keywords);
