			...
			<div class="result-content">
				<table class="result-tab" width="100%">
					...
					<c:forEach var="goods" items="${goodslist}">
						<tr>
							<td class="tc"><input name="id[]" value="${goods.GOODS_ID }" type="checkbox"></td>
							<td>${goods.GOODS_ID}</td>
							<td>${goods.GOODS_TITLE}</td>
							<td>${goods.GOODS_DESC}</td>
							<td>${goods.GOODS_IMAGES}</td>
							<td>${goods.GOODS_PRICE}</td>
							<td>${goods.GOODS_COUNT}</td>
							<td>${goods.GOODS_TYPE}</td>
							<td>${goods.GOODS_PUBLISHER}</td>
							<td>${goods.GOODS_ADDRESS}</td>
							<td>${goods.GOODS_CONTACT}</td>
							<td>${goods.GOODS_STATE}</td>
							

							<td>
								<a class="link-update" href="admin_togoodsupdate?id=${goods.GOODS_ID}&page=${cpage}${search}">修改</a> 
								<a class="link-del" href="javascript:Delete('你确定要删除商品【${goods.GOODS_TITLE }】吗?', '/testshop/manage/admin_dogoodsdelete?id=${goods.GOODS_ID}&page=${cpage}${search}')">删除</a>
							</td>
						</tr>
					</c:forEach>
