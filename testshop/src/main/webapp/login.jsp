	...
	<script>
	 ...
	 $(function(){
		 $('#login').click(function(){
			...
			var url="js/reg_userAccount.js?account="+account+"&password="+password;
			var args ={"time":new Date()};//这里是点击登录功能，对应接口文档功能二
			$.getJSON(url,args,function(data){
				if(data.match=="true"){
					var userID=data.userID;
					var url="js/reg_userAccount.js?userID="+userID;
					$.getJSON(url,args,function(user_data){
					 userID=user_data.userID;
					 var account=user_data.account;
					 var identity=user_data.identity;
					 ...
					 if(!window.localStorage){
				            ...
				            return false;
				        	 ...
				        	 storage["icon"]=user_data.icon;
				        	 storage["account"]=user_data.account;
				        	 storage["contact"]=user_data.contact;
				        	 storage["address"]=user_data.address;
				        	 storage["email"]=user_data.email;
				        	 storage["username"]=user_data.username;
				        	 storage["stunum"]=user_data.stunum;
				            //主逻辑业务
				        }
					})
			...
			case "userAccount" :
				...
				if(p_value==""){
					...
					if(!(p_value.length>5&&p_value.length<17) ){
						...
						$(function(){
						var url="js/reg_userAccount.js?account="+p_value;//这一部分是判断账号是否存在 对接文档功能1
