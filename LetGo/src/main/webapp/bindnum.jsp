...
<div class="Bott">
    <div class="wrapper clearfix">
        ...
        <div class="you fl"><h2>卖家认证</h2>
            <form onsubmit="return false" action="#" method="post" class="remima">
            	 
                <p><span>学号：</span><input id="stun1" name="Stu" type="text" value="" oninput="myFunction()"/><span class="error">请输入学号</span></p>
                
                <p><span>邮箱：</span><input id="stun2" name="userEmail" type="text" value="" readonly="readonly"/></p>
              ...
              disable="" onclick=settime(this); id="repwd1">发送验证码</button><span></span></p>
                ...
                <p><span>联系方式(QQ)：</span><input id="cont" name="Cont" type="text" value=""/><span class="error">请输入联系方式</span></p>
...
window.onload = function(){
	var url="IsStuNumBind";
	var args ={"userID":id};//这里是验证是否已绑定学号功能，对应接口文档功能9
	...
	$.post(url,args,function(data){ 			
		 if(data.found=="true"){
			 ...
			 $("#addr").val(address);
			 $("#cont").val(contact);
			 $("#stun1").eq(0).next().empty().append("您已绑定学号");
			 $("#stun1").eq(0).next().text();
			 $("#stun2").eq(0).next().empty();
			 $("#stun3").eq(0).next().next().empty();
			 $("#addr").eq(0).next().empty();
			 $("#cont").eq(0).next().empty();
			 $("#stun1").attr("disabled","disabled");
			 $("#repwd1").attr("disabled","disabled"); 
			 $("#stun3").attr("disabled","disabled"); 
...
$(function(){
	...
	var args ={"userID":id,
				"stuNum":stuNum,
				"address":address,
				"contact":contact};//这里是点击卖家认证功能，对应接口文档功能13
	$.post(url,args,function(data){
...
$(function(){
	$("button").click(function(){
		var c=$("#stun1").eq(0).next().text();
		var p_value=$("#stun2").eq(0).val();
		if(c==""){
			var url="SendEmailCaptcha";//发送email按钮然后接受验证码,对应功能11
...
$("input").blur(function(){
		...
		if(p_value==""){
		 ...
		 $(function(){
			 var url="CheckStuNum";
				...
				var args ={"stuNum":$("#stun1").eq(0).val()};//这里是验证学号是否被绑定功能，对应接口文档功能10

				$.post(url,args,function(data){
						if(data.exist!="true"){
							$("#stun1").eq(0).next().empty();
							$("#stun2").eq(0).val($("#stun1").eq(0).val()+"@whu.edu.cn");
							$.cookie('stuNum', $("#stun1").eq(0).val());
							$("#repwd1").removeAttr("disabled");
							 $("#repwd1").css({background:"#C10000"});
							 
						}else{
							$("input").eq(1).next().empty().append("此学号已被绑定");
							$("#repwd1").attr("disabled","disabled"); 
	...
	case "code" :
		...
		if(captcha==""){
			...
			var url="VerifyCaptcha"; //这里是后台验证验证码是否正确 对应功能12
