			...
			<div class="result-content">
				<table class="result-tab" width="100%">
					...
					<c:forEach var="cart" items="${cartlist}">
						<tr>
							<td class="tc"><input name="id[]" value="${cart.id }" type="checkbox"></td>
							<td>${cart.id}</td>
							<td>${cart.userId}</td>
							<td>${cart.goodsId}</td>

							<td>
								<a class="link-update" href="admin_tocartupdate?id=${cart.id}&page=${cpage}${search}">修改</a> 
								<a class="link-del" href="javascript:Delete('你确定要删除购物车【${cart.id }】吗?', '/testshop/manage/admin_docartdelete?id=${cart.id}&page=${cpage}${search}')">删除</a>
							</td>
						</tr>
					</c:forEach>
